...
class ChefEngine < Engine
  def build_step
    ...
    metadata_str = ChefHelper.read_repo_metadata(@source_git_local_path)
    chef_metadata = ChefHelper.parse_metadata(metadata_str)
    ...
    ChefHelper.write_repo_metadata(@source_git_local_path, new_metadata_str)
  ...
  def package_step
    ...
    GitUtils.commit(@source_git_local_path, "(v#{next_version}) Automated packaging of release by CapsuleCD")
    @source_release_commit = GitUtils.tag(@source_git_local_path, "v#{next_version}")
  end
